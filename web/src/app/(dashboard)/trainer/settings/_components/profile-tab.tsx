...
interface TrainerProfileTabState {
  avatarUrl: string | null;
  saving: boolean;
...
function trainerProfileTabReducer(state: TrainerProfileTabState, action: TrainerProfileTabAction): TrainerProfileTabState {
  switch (action.type) {
...
export function TrainerProfileTab(props: Props) {
  ...
  const [state, dispatch] = useReducer(trainerProfileTabReducer, undefined, () => ({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="space-y-1.5">
        <label htmlFor="bio" className="text-[11px] font-semibold uppercase tracking-[1.5px] text-foreground/65">Bio</label>
        <Textarea
          id="bio"
          name="bio"
      ...
      <div className="space-y-1.5">
        <label htmlFor="specializations" className="text-[11px] font-semibold uppercase tracking-[1.5px] text-foreground/65">Specializations</label>
        <Input
          id="specializations"
          name="specializations"
          defaultValue={props.specializations.join(', ')}
          placeholder="strength, rehabilitation, nutrition (comma-separated)"
          className="bg-card border-foreground/10 text-foreground"
        />
        <p className="text-xs text-foreground/65">Separate multiple specializations with commas</p>
      </div>
