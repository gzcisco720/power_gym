...
import type { FoodItem, FoodMacros, FoodServing } from '@/api/foods';
...
const OPTIONAL_MICROS: { key: keyof FoodMacros; label: string }[] = [
...
const SECTION_LABEL = 'text-[11px] font-semibold uppercase tracking-wider text-foreground/65';
...
export function FoodForm({ mode = 'create', initialFood, onSaved, onCancel, stickyFooter = false }: FoodFormProps) {
  const storeAction = useFoodsStore((s) => mode === 'edit' ? s.update : null);
  const storeSave = useFoodsStore((s) => s.save);
  ...
  const isDirty = useMemo(
    () => JSON.stringify({ name, brand, kcal, protein, carbs, fat, micros, servings }) !== initialSnapshot,
  ...
  async function handleSubmit(e: React.FormEvent): Promise<void> {
    ...
    const parsedServings = validServings.map((s) => ({ label: s.label.trim(), grams: parseFloat(s.grams) }));
    ...
    const macrosPer100g: FoodMacros = { kcal: parsedKcal, protein: parsedProtein, carbs: parsedCarbs, fat: parsedFat };
    for (const { key } of OPTIONAL_MICROS) {
      const val = micros[key];
      if (val && val.trim() !== '') {
        const n = parseFloat(val);
        if (!isNaN(n)) (macrosPer100g as unknown as Record<string, number>)[key] = n;
      }
    }

    const payload = { name: name.trim(), brand: brand.trim() || null, macrosPer100g, servings: parsedServings };

    dispatch({ type: 'SET_SUBMITTING', value: true });
    try {
      let food: FoodItem;
      if (mode === 'edit' && initialFood && storeAction) {
        food = await storeAction(initialFood._id, payload);
      } else {
        food = await storeSave(payload);
      }
      toast.success(mode === 'edit' ? 'Food saved' : 'Food created');
      onSaved(food);
    } catch {
      const msg = `Failed to ${mode === 'edit' ? 'save' : 'create'} food.`;
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className={stickyFooter ? 'space-y-4 pb-4' : 'space-y-4'} noValidate>
        {/* Basic info */}
        <div className="rounded-xl bg-card ring-1 ring-foreground/10 p-4 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input id="food-name" value={name} onChange={(e) => dispatch({ type: 'SET_NAME', value: e.target.value })} placeholder="e.g. Chicken Breast" required />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <Input id="food-brand" value={brand} onChange={(e) => dispatch({ type: 'SET_BRAND', value: e.target.value })} placeholder="e.g. Generic" />
          </div>
        </div>
        ...
        <div className="rounded-xl bg-card ring-1 ring-foreground/10 p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {(
              [
                { key: 'kcal', label: 'Energy (kcal)', val: kcal, set: (v: string) => dispatch({ type: 'SET_KCAL', value: v }) },
                { key: 'protein', label: 'Protein (g)', val: protein, set: (v: string) => dispatch({ type: 'SET_PROTEIN', value: v }) },
                { key: 'carbs', label: 'Carbs (g)', val: carbs, set: (v: string) => dispatch({ type: 'SET_CARBS', value: v }) },
                { key: 'fat', label: 'Fat (g)', val: fat, set: (v: string) => dispatch({ type: 'SET_FAT', value: v }) },
              ] as const
        ...
        <div className="rounded-xl bg-card ring-1 ring-foreground/10 p-4 space-y-3">
          <div className="flex items-baseline justify-between">
            ...
            <Button type="button" variant="ghost" size="sm" onClick={addServing} className="h-7 text-xs text-foreground/65 hover:text-foreground">
        ...
        <div className={footerClass}>
          <Button type="button" variant="outline" onClick={handleCancelClick} disabled={submitting} className="border-foreground/20 hover:border-foreground/40 hover:bg-muted">
            Cancel
          </Button>
      ...
      <Dialog open={pendingCancel} onOpenChange={(o) => { if (!o) dispatch({ type: 'SET_PENDING_CANCEL', value: false }); }}>
        <DialogContent className="sm:max-w-md" showCloseButton={false}>
          <DialogHeader>
            <DialogTitle>Discard changes?</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-foreground/65">You have unsaved changes. Leaving will discard them.</p>
          <div className="flex gap-2 justify-end pt-2">
            <Button type="button" variant="outline" onClick={() => dispatch({ type: 'SET_PENDING_CANCEL', value: false })}>
              Keep editing
            </Button>
            <Button type="button" variant="destructive" onClick={() => { dispatch({ type: 'SET_PENDING_CANCEL', value: false }); onCancel(); }}>
              Discard
            </Button>
          </div>
        </DialogContent>
