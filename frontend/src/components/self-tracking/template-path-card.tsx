  ...
  async function handleLog(template: UserTemplate, day: UserTemplateDay, deleteActive = false) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        navigate(`${basePath}/session/${log._id}`);
        return;
      }
      if (res.status === 409) {
  ...
  if (templates.length === 0) {
    return (
      <div className="rounded-xl bg-card ring-1 ring-foreground/10 p-4 flex flex-col">
        ...
        <Button
          variant="outline"
          onClick={() => navigate(basePath.replace('/my-training', '/plans/new'))}
        >
          + Create Template
        </Button>
      </div>
            ...
            return (
              <div key={tpl._id} className="rounded-lg ring-1 ring-foreground/10 overflow-hidden">
                <button
                  ...
                  onClick={() =>
                    dispatch({ type: 'SET_EXPANDED_ID', value: isExpanded ? null : tpl._id })
                  }
                    ...
                    {tpl.days.map((day) => (
                      ...
                      >
                        ...
                        <Button
                          ...
                          variant="ghost"
                          disabled={starting}
                          onClick={() => void handleLog(tpl, day)}
