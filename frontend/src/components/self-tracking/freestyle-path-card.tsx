  ...
  async function startBlank(deleteActive = false) {
    ...
    try {
      const url = deleteActive
        ...
        : '/api/me/workout-logs';
      try {
        const log = await request<{ _id: string }>(url, {
          method: 'POST',
          body: JSON.stringify({ dayName: 'Freestyle', plannedSets: [] }),
        });
        navigate(`${props.basePath}/session/${log._id}`);
      } catch (err) {
        const errMsg = err instanceof Error ? err.message : '';
        // Check for 409 conflict by re-trying with raw fetch
        if (errMsg.includes('409')) {
          // Re-fetch with raw to get the body
          ...
          if (rawRes.status === 409) {
            const body = (await rawRes.json()) as ConflictBody;
    ...
    <>
      <div className="rounded-xl bg-card ring-1 ring-foreground/10 p-4 flex flex-col">
        ...
        {props.state === 'full' && (
          <div className="text-[11px] text-foreground/65 mb-3 tabular-nums">
            Recent freestyle frequency:{' '}
            <span className="text-foreground">{props.weeklyFrequency} / week</span>
          </div>
        )}

        <div className="mt-auto">
          <Button onClick={() => void startBlank()} disabled={starting} className="w-full">
      ...
      {dayAlreadyLogged && (
        <DayAlreadyLoggedDialog
          ...
          onDeleteLog={async () => {
            try {
              await fetch(`/api/me/workout-logs/${dayAlreadyLogged._id}`, {
                method: 'DELETE',
                credentials: 'include',
              });
