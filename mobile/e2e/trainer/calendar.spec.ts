/**
 * Trainer Calendar E2E — Detox on iOS Simulator
 *
 ...
 *
 * 3. Run the E2E tests:
 *    cd mobile && pnpm detox:test --testPathPattern=trainer/calendar
 *
 ...
 *
 * Seeds:
 *   - A fresh trainer account per run
...
const TS = Date.now();
const TRAINER_EMAIL = `trainer-calendar-${TS}@powergym.com`;
const TRAINER_PASSWORD = 'TrainerPass123!';
const API_URL = process.env.EXPO_PUBLIC_API_URL ?? 'http://localhost:3001';

describe('Trainer: Calendar', () => {
  beforeAll(async () => {
    // Seed trainer account with a member so we can assign them to a session
  ...
  it('golden path: open member Select, select single member, fill date/start/end, save → session card appears', async () => {
    // Tap + Add button
    ...
    // Tap the first available member item in the Select content
