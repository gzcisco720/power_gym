/**
 * Member My Training E2E — Detox on iOS Simulator
 *
 ...
 *
 * 3. Run the E2E tests:
 *    cd mobile && pnpm detox:test --testPathPattern=member/my-training
 *
 ...
 *   - Assigns the template to the member via /training/dev/seed
 */
...
const MEMBER_EMAIL = `member-training-${TS}@powergym.com`;
...
describe('Member: My Training', () => {
  beforeAll(async () => {
    ...
    // Seed member (we'll assign them to the owner below)
    await fetch(`${API_URL}/auth/dev/seed-user-role`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    // Seed the training plan for the member
  ...
  describe('edge case: no plan assigned shows empty state', () => {
    const EMPTY_MEMBER_EMAIL = `member-noplan-${TS}@powergym.com`;
    const EMPTY_MEMBER_PASSWORD = 'MemberPass123!';

    beforeAll(async () => {
      // Seed a member with no plan assigned
      await fetch(`${API_URL}/auth/dev/seed-user-role`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
