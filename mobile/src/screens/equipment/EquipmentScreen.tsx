...
import { FilterTab } from '../../types/equipment';
    ...
    <Screen testID="screen-Equipment">
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        <View className="px-4 py-4 gap-2">
          {loading ? (
            <Text className="text-[13px] text-foreground/65 text-center mt-8">Loading...</Text>
          ) : filtered.length === 0 ? (
            renderEmpty()
          ) : (
            filtered.map((item) => (
              <EquipmentCard key={item._id} item={item} onPress={() => {}} />
            ))
          )}
        </View>
      </ScrollView>
