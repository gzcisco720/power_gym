...
| `design-reviewer` | Checks any UI change (`web/` or `mobile/`) against design guidelines — reports violations only | `use the design-reviewer agent on path/to/component` |
...
| `/simplify` + `react-doctor` | ✅ Keep | The Refactor step after every Green phase (`react-doctor` for `web/` only) |
...
```
use the design-reviewer agent on path/to/changed/component
```

Run after the Evaluator passes a Stage that contains UI work (`web/` or `mobile/`). The design-reviewer checks color tokens, spacing, form patterns, accessibility, and compares against any design documents in `.superpowers/`.
...
| Mobile app source, configs, tests | `mobile/` |
| Backend source, configs, tests | `backend/` |
...
| App | Dev command | Test | E2E |
| --- | --- | --- | --- |
| `web/` | `cd web && pnpm dev` | `cd web && pnpm test` | `cd web && pnpm test:e2e` |
| `mobile/` | `cd mobile && pnpm start` | `cd mobile && pnpm test` | `cd mobile && pnpm detox test --configuration <config>` |
| `backend/` | `cd backend && pnpm start:dev` | `cd backend && pnpm test` | `cd backend && pnpm test:e2e` |
